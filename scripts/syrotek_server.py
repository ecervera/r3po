...
class SyrServer:
	def __init__(self):
		self.server = actionlib.ActionServer('syr_run_script', RunScriptAction, self.execute, self.cancel, False)
		self.goalHandle = {}
		self.goal = {}
		self.pm = {}
		self.cancelled = {}
	...
	def execute(self, goalHandle):
		...
		self.filename[bot] = '/home/ecervera/catkin_ws/src/r3po/sandbox/syrotek/'+ modulename
		...
		with open(self.filename[bot]+'.output', "w") as text_file:
			self.pm[bot] = subprocess.Popen('rosrun r3po sandbox/syrotek/' + modulename+'.py',
									stdout=text_file,
...
if __name__ == '__main__':
	global server
	rospy.init_node('syrotek_server')
	rospy.loginfo("Starting syrotek_server...")
	server = SyrServer()
