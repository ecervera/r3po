...
def handle_acquire(req):
	...
	if world.find('multi')==-1:
		...
		stage_proc[stage] = []
		stage_proc[stage].append(subprocess.Popen('rosrun stage stageros -g $(rospack find r3po)/stage/' + world + ' /clock:=clock',
										env=my_env,
										shell=True))
		stage_proc[stage].append(subprocess.Popen('rosrun r3po robot_pose.py',env=my_env,shell=True))
	else:
		my_env = os.environ
		my_env["ROS_NAMESPACE"] = stage
		stage_proc[stage] = []
		...
		stage_proc[stage].append(subprocess.Popen('rosrun stage stageros -g $(rospack find r3po)/stage/' + world + ' /clock:=clock' + remap,
										env=my_env,
										shell=True))
		for robot in range(3):		
			my_env["ROS_NAMESPACE"] = stage + ('/robot_%s' % robot)
			stage_proc[stage].append(subprocess.Popen('rosrun r3po robot_pose.py',env=my_env,shell=True))
		#my_env["ROS_NAMESPACE"] = stage + '/robot_1'
		#stage_proc[stage].append(subprocess.Popen('rosrun r3po robot_pose.py',env=my_env,shell=True))
		#my_env["ROS_NAMESPACE"] = stage + '/robot_2'
		#stage_proc[stage].append(subprocess.Popen('rosrun r3po robot_pose.py',env=my_env,shell=True))
